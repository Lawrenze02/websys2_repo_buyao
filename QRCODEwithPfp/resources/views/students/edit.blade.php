...
<div class="page-header">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            ...
            <p>Update information for <strong>{{ $student->full_name }}</strong></p>
        </div>
    </div>
</div>
            ...
            <form action="{{ route('students.update', $student) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <small class="text-muted">Leave blank to keep current photo. Max 2MB.</small>
                        @error('profile_photo')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mt-5 d-flex gap-2">
                    ...
                    <a href="{{ route('students.show', $student) }}" class="btn btn-outline-secondary px-4 py-2">Cancel</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="col-lg-4">
        <div class="card mb-4 overflow-hidden">
            <div class="card-header">
                <h6 class="mb-0 fw-bold">Current Photo</h6>
            </div>
            <div class="card-body text-center py-4 bg-light">
                @if($student->photo_url)
                    <img src="{{ $student->photo_url }}" class="img-thumbnail" style="width:150px; height:150px; object-fit:cover; border-radius:20px;">
                @else
                    <div class="avatar-placeholder-lg mx-auto">{{ $student->initials }}</div>
                    <p class="text-muted mt-3 mb-0">No photo uploaded</p>
                @endif
            </div>
        </div>
    </div>
